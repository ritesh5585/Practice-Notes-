...
Using `AbortController`s involves the following steps:
...
- `AbortController`s are not `fetch()`-specific, they can be used to abort other asynchronous tasks as well.
