...
- Run scripts in background threads separate from the main UI thread.
- Designed for CPU-intensive tasks like data processing, mathematical computations, etc. Generally for non-async work.
...
- The service worker listens for the `fetch` event, which is fired whenever the browser makes a network request within the worker's scope.
