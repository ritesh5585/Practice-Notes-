...
As mentioned above, ES2015 introduced [arrow functions](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Functions/Arrow_functions), which use the [enclosing lexical scope](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Functions/Arrow_functions#No_separate_this). This is usually convenient, but it does prevent the caller from defining the `this` context via `.call`/`.apply`/`.bind`. One of the consequences is that DOM event handlers will not properly bind `this` in your event handler functions if you define the callback parameters to `.addEventListener()` using arrow functions.
